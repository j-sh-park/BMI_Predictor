...
```{r}
t.test(data$Height, mu = avg_h, alternative = "two.sided")
```
...
```{r}
k = 10 
lasso_mse = simple_hip_mse = simple_mse = full_mse = vector(mode = "numeric", length = k) 
lasso_mae = simple_hip_mae = simple_mae = full_mae = vector(mode = "numeric", length = k)
```
...
```{r}
cv_res = tibble(simple_mse, full_mse, simple_mae, full_mae, simple_hip_mse, simple_hip_mae, lasso_mse,lasso_mae) 
cv_res
```

```{r}
result = matrix(c(sqrt(mean(simple_mse)),sqrt(mean(simple_hip_mse)),sqrt(mean(full_mse)), sqrt(mean(lasso_mse)), mean(simple_mae), mean(simple_hip_mae),mean(full_mae),mean(lasso_mae)), ncol=2)

colnames(result)= c('RMSE', 'MAE')
rownames(result) = c('simple model', 'simple model with log hip', 'full model','lasso')
